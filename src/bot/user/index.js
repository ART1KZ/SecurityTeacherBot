...
async function performSearch(query) {
    try {
        ...
        if (!chunks || chunks.length === 0) {
            return {
                answer: "Извините, не нашёл информации по вашему запросу. Попробуйте переформулировать вопрос.",
                source: null,
                probability: null,
            };
        }
        ...
        const context = topChunk.text;

        await delay(400);

        // console.log(context, "\n\n\n", query);
        // console.log(`Контекст: ${context} Вопрос: ${query}`)
                ...
                messages: [
                    {
                        role: "system",
                        content:
                            "Отвечай на вопросы только на основе информации из предоставленного контекста." +
                            "Если контекст хоть немного близок к теме, напиши:" +
                            "В контексте нет ответа конкретно на ваш вопрос, однако есть информация о *информация из контекста*",
                    },
                    {
                        role: "user",
                        content: `Контекст: ${context} Вопрос: ${query}`,
                    },
                ],
            });
        });
        ...
        return {
            ...
            probability:
                topChunk.similarity || topChunk.similarity_probability || null,
        };
    } catch (err) {
        ...
        if (
            ...
            return {
                answer: "⚠️ Превышен лимит запросов к API. Пожалуйста, подождите минуту и попробуйте снова.",
                source: null,
                probability: null,
            };
        }
...
userComposer.on("message:text", async (ctx) => {
    if (ctx.session.command === "search") {
        ...
        if (source) {
            ...
            keyboard
                .text(
                    "📥 Скачать документ-источник",
                    `user_download_doc_${String(source)}`
                )
                .row();
        }
